...
| M3 inferlite | `BatchedKVCache` gather | ✅ 63% | ✅ 22% | ✅ 15% | 是（纯 PyTorch） |
| nano-vllm | PyTorch gather + per-row mask | ✅ | ✅ | ✅ | 是（和我们一样） |
| vLLM | PagedAttention CUDA kernel | 否（kernel 内） | 否（kernel 内） | 否 | 否 |
| SGLang | RadixAttention + 自定义 kernel | 否 | 否 | 否 | 否 |

**教学版（inferlite M3 / nano-vllm）都有这个限制**——纯 PyTorch 的 batched decode 比 single decode 慢，直到用自定义 kernel 才解决。
...
性能优化留给后续里程碑：
- **M4 PagedAttention**：用 block_table + 按需 gather，避免 materialize 整个 dense tensor（但 for 循环 + Python 开销还在，PyTorch 版仍受限）
- **M8 Triton kernel**：把 for 循环 + gather + mask 下沉到 kernel 内部，彻底消除 Python 开销和同步
