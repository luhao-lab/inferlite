...
针对将要开始的里程碑 `$ARGUMENTS`（如 M2），**带着前期积累 + 调研简报**生成完整 plan。

## 0. 前置调研（必跑）

```
/research-before-plan $ARGUMENTS
```

生成 `~/learning/docs/projects/inferlite/research/$ARGUMENTS-brief.md` 和补全 knowledge 卡。**没有 brief 不允许进入 step 1**。

## 1. 收集上下文

### 1.1 调研简报（最高优先）
```
read_file ~/learning/docs/projects/inferlite/research/$ARGUMENTS-brief.md
```
**这是 plan 的事实基础**，关键发现/风险/边界都要进 plan。
...
### 1.6 Knowledge 卡（已由 research-before-plan 补全）
```
ls ~/learning/docs/projects/inferlite/knowledge/{papers,libs,concepts,tools}/ | grep -i <主题>
```
...
- "前置"区块必须包含**前置 knowledge 卡链接**（来自 step 1.6）
- "坑"区块**从 lessons/ + research brief 风险预判 引用**，不要重新发明
