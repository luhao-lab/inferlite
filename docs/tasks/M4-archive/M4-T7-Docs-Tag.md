...
> 在 T4～T6 代码、E2E 和 benchmark 都完成后，做 M4 里程碑闭环：更新 README / PROGRESS / M4 plan / knowledge，检查任务卡完成总结和最终门禁，准备 annotated tag `m4/paged-attention`。
...
T7 是 **文档与里程碑收口任务**，不再实现新能力。它的职责是把 T4～T6 已经完成并验证过的事实沉淀到项目文档、进度记录和 tag 中。
...
- 检查 M4-T1～T7 每张任务卡是否有完成总结和 commit 号。
...
- 不修核心实现 bug；发现 bug 回退到对应 T4/T5/T6。
- 不新增 Prefix Cache 设计细节；最多说明 M4 为 M5 准备了 BlockPool/ref_count/block table 基础。
- 不把未完成项口头标成完成。
...
```text
BlockPool -> BlockTable -> PagedKVCache -> PagedAttention -> PagedEngine
```
...
- PyTorch gather 伪版与 M9 kernel 的关系。
- M5 Prefix Cache 将复用哪些基础设施。

### 5. Lessons

如 T4～T6 出现可复用坑，更新 `docs/knowledge/lessons.md`。候选：
...
T7 不伪造 commit。若某任务未提交，标明「待提交」并不得创建最终 tag。
...
1. 汇总 T4～T6 完成总结、验证命令、commit 和 benchmark 结果路径。
2. 更新 README 当前进度。
3. 更新 `docs/plan/PROGRESS.md`。
4. 更新 `docs/plan/M4.md`。
5. 更新 knowledge / lessons。
6. 检查 M4-T1～T7 任务卡完成总结完整性。
7. 跑最终门禁。
8. 确认工作区与 commit 状态。
9. 创建 annotated tag。
10. T7 任务卡追加完成总结与 tag 信息。
...
- [ ] knowledge 文档解释完整 M4 数据流和 benchmark 结论。
- [ ] lessons 如有新增教训已更新。
- [ ] M4-T1～T7 每张任务卡都有完成总结和 commit 号。
...
1. **发现 bug 但在 T7 顺手修**：T7 只能收口，bug 回退到对应任务，避免文档任务混入实现变更。
...
6. **过度展开 M5 设计**：T7 只说明 M4 为 M5 准备了基础，不提前写 Prefix Cache 实现细节。
7. **tag 前工作区不干净**：必须确认 commit 状态，避免 tag 指向半成品。
8. **未记录环境限制**：若某些命令无法跑，必须明确原因和风险。
