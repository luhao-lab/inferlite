...
本卡完成了 M3 的固定槽位多请求 KV Cache：

- `BatchedLayerKVCache`：单层 cache，shape `[max_num_slots, n_kv_heads, max_seq_len, head_dim]`。
- `SlotManager`：用 `deque` 管理 `free_slots`，用 `req_to_slot` 维护 `request_id → slot_id` 唯一映射。
- `BatchedKVCache`：组装多层 cache + `seq_lens` + `occupied` + `SlotManager`，提供 `from_config()` 工厂方法。

关键设计结论：

- `free_slots` 用 `deque`，`popleft` O(1)，分配顺序从低 slot id 开始。
- 只保留 `req_to_slot` 单向映射，调用方有 `request_id`，`free` 时直接传 `request_id` 即可。
...
- 不做 PagedAttention（留 M4）。
- 不做 block table / prefix cache / session cache / TTL / LRU / eviction。
- 不改 attention kernel，只做 cache 管理。
- 不修改 M2 的 `KVCache`，新增文件，M2 保持兼容。

对后续任务的接口关系：

- T3 的 `GQAAttention.forward` 新增 `cache_slots` / `cache_positions` 参数，用 `BatchedKVCache` 的 slot id 做 per-row cache 读写。
- T4 的 `batch_generate` 在 prefill 时调用 `slot_manager.allocate(req.request_id)`，在 finished 时调用 `free(req.request_id)`。
- `max_num_slots` 同时是预分配槽位总数和最大 running 请求数，`cache_slots` 是当前 decode step 参与 batch 的 slot id 列表。
