...
本文档记录 M3 的完整设计决策、实现细节和总结回顾。设计部分（§1–§9）记录每个任务卡的设计决策和代码细节；总结部分（§10–§14）汇总测试覆盖、benchmark 结果、局限性分析和后续路径。
...
## 4. T2: BatchedKVCache + SlotManager
