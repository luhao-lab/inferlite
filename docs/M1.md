...
> 完整模板：**前置 / 产出 / 算法核心 / L0 测试 / DoD / 坑**
> 这里给紧凑版索引，每张卡的详细描述在 §6 单卡章节里展开（开始做一张时再读那张）。

| # | 名称 | 状态 | 前置 | 产出文件 | L0 测试 | 估时 |
| --- | --- | --- | --- | --- | --- | --- |
| T0 | 包骨架 + preflight | ✅ | — | `scripts/setup.sh`, `scripts/preflight.py`, `inferlite/__init__.py` | 跑通 preflight | 已完成 |
| T0' | ModelConfig dataclass | ⬜ | T0 | `inferlite/config.py` | `test_config.py`：11 字段全测 | 20m |
| **T1** | **RMSNorm** | **✅** | T0' | `inferlite/model/layers.py::RMSNorm` | `tests/unit/test_rmsnorm.py` 12/12 | 已完成 |
| T2 | SwiGLU MLP | ⬜ | T0' | `inferlite/model/layers.py::SwiGLUMLP` | `tests/unit/test_mlp.py` (≥6 cases) | 1h |
| T3 | RoPE | ⬜ | T0' | `inferlite/model/layers.py::RotaryEmbedding`, `apply_rotary_pos_emb` | `tests/unit/test_rope.py` 3 子测试 | 2h |
| T4 | GQA Attention | ⬜ | T1+T3 | `inferlite/model/layers.py::GQAAttention` | `tests/unit/test_attention.py` 4 子测试 | 3h |
| T5 | DecoderLayer | ⬜ | T1+T2+T4 | `inferlite/model/qwen3.py::Qwen3DecoderLayer` | `tests/unit/test_decoder_layer.py` | 1h |
| T6 | Qwen3Model | ⬜ | T5+T1 | `inferlite/model/qwen3.py::Qwen3Model` | 形状测试 + 随机权重 forward | 2h |
| T7 | WeightMap + load_from_hf | ⬜ | T6 | `inferlite/model/weight_map.py`, `inferlite/model/loader.py` | `tests/unit/test_weight_map.py`（key 映射 + tie_embedding） | 3h |
| **T8** | **L1 logits 对齐** | ⬜ | T7 | `tests/module/test_qwen3_logits.py` | 整模型 allclose vs transformers | 2h |

### 任务卡总表（M1b）

| # | 名称 | 状态 | 前置 | 产出文件 | 测试 | 估时 |
| --- | --- | --- | --- | --- | --- | --- |
| T9 | LLMModel Protocol + GreedySampler | ⬜ | T8 | `inferlite/model/protocol.py`, `inferlite/sampler/{protocol,greedy}.py` | `test_greedy_sampler.py` | 1h |
| T10 | EngineCore.step() 三段式 | ⬜ | T9 | `inferlite/engine/core.py`, `inferlite/scheduler/trivial.py`, `inferlite/executor/uniproc.py` | `tests/invariant/test_step_three_phase.py` | 3h |
| T11 | CLI + L2 e2e | ⬜ | T10 | `inferlite/cli.py` | `tests/e2e/test_greedy_decode.py` | 2h |
...
## 6. 单卡详细模板（每张卡按这个 7 字段写）

> 篇幅原因，下面只展开 T2（下一张要做的），其余卡片在开始做时再写到本文件 §6.x。
...
- **产出**：
  ...
  - `tests/unit/test_mlp.py`（≥6 case）
- **算法核心**（4 行）：
  ```python
  gate = self.gate_proj(x)        # [..., I=3072]
  up   = self.up_proj(x)          # [..., I]
  hidden = F.silu(gate) * up      # element-wise gating
  return self.down_proj(hidden)   # [..., H=1024]
  ```
- **L0 测试**：
  | # | 测什么 | 容差 |
  | --- | --- | --- |
  | 1 | vs `Qwen3MLP`（3 dtype） | fp32 1e-5 / fp16/bf16 5e-3 |
  | 2 | 无 bias | exact |
  | 3 | shape 不变 | exact |
  | 4 | 恰好 3 个 nn.Linear | exact |
- **DoD**：测试 6/6 绿 + commit `feat(model): SwiGLUMLP (T2 done)`
- **坑**：
  - 默认 `nn.Linear(..., bias=True)` 必须改 `bias=False`
  - `F.silu(gate) * up` 不是 `F.silu(gate * up)`
  - 自己写 silu = `x * sigmoid(x)` 也对但慢，用 `F.silu`
