...
<!-- anchor:milestone-overview -->
## 0.2 按功能维度的里程碑速览（2026-07-19 补）

> 上面 §0/§1 是按"技术深度 + 4 层抽象"组织，适合"知道在学什么"。
> 这一节按**功能维度**重组，回答"知道在做什么"——inferlite 要做成一个什么样的系统。

### 一句话心智模型

**把"一个 LLM 请求"变成"一个可服务的系统"**——从单条推理，到多请求调度，到显存管理，到对外服务，再到能力扩展。

### 4 大功能主题

| 主题 | 里程碑 | 抽象目标 | 具体功能（用户能看到什么） |
|------|--------|----------|--------------------------|
| **A. 多请求共享算力** | M3-M4 | 把孤立请求变成挤进同一次前向的并发系统 | 8 请求同时生成，短请求完成让 slot 给等待请求；显存不浪费，长 prompt + 多并发不 OOM，前缀能复用 |
| **B. 对外服务** | M5 | 把 Python 函数变成网络服务 | `inferlite serve` 起服务，curl 流式输出像 ChatGPT；多轮对话第二轮秒回（前缀缓存）；对照表 + CI + v1.0 tag |
| **C. 支持更多模型类型** | M6, M9, M13, M14 | 从只会跑 Qwen3 dense 到能跑各种模型 | MoE（Qwen3-30B-A3B 激活 3B）、多模态（图文对话）、MoE 工程化（grouped GEMM 5+ tok/s）、VLM 工程化（多分辨率 + 图片前缀缓存） |
| **D. 加速能力与长上下文** | M7, M8, M10, M11, M12 | 让推理更快、能吃更长上下文 | n-gram spec（1.5×）、Triton kernel（≈ vLLM，需 GPU）、EAGLE spec（3×）、chunked prefill（32K+ 不 OOM）、YaRN（128K 上下文） |
...
M1-M2（已完成）
    ↓
主题 A (M3-M4)  ──必需──> 主题 B (M5，v1.0 收口)
                    └──部分──> 主题 D·M8（M4 是 M8 的替换对象）
                              ↓
                         主题 C/D (M6+，平行扩充)
```
...
| 依赖 | 类型 | 说明 |
|------|------|------|
| A → B（M3-M4 必需先于 M5） | 硬依赖 | M5·P2 Prefix Cache 依赖 M4 的 block_table；M5·P1 API 要包 batch_generate（M3） |
| A → D·M8（M4 先于 M8） | 硬依赖 | M8 Triton kernel 替换 M4 的 PyTorch 伪版，没 M4 没替换对象 |
| B → C/D（M5 先于 M6+） | 软依赖 | M5 是 v1.0 收口，M6+ 是扩充。实践中"先有完整服务再扩能力"更稳，但理论上 M6 不依赖 M5 |
| C 和 D 内部 | 部分依赖 | M9 依赖 M6（MoE 升级），M10 依赖 M7（EAGLE 升级），M14 依赖 M13（VLM 升级） |
| C 和 D 之间 | 无依赖 | 多模型/多模态 和 加速/长上下文 互不依赖，可平行 |

### 推荐节奏：集中攻克 A，收口 B，自由探索 C/D

**阶段 1（集中）：M3（收尾）→ M4** — 主题 A，约 2-3 周
- M3 的 BatchedKVCache 和 M4 的 PagedKVCache 是同一条 cache 路径的演进，心智模型连续
- M3-M4 是后续所有主题的地基，不集中做完会反复返工

**阶段 2（收口）：M5·P1 → P2 → P3** — 主题 B，约 2-3 周，打 v1.0 tag
- M5 是 v1.0 收口点，《2000 行实现一个 vLLM》宣传文的载体
- 自然停顿点，适合回头巩固

**阶段 3（自由）：按兴趣选 M6/M7/M11/M13 等** — 主题 C/D，无截止
- C 和 D 互不依赖，每个 M 都是独立能力，做完一个就能用
- 不需要"集中攻克"，按演示价值 / 硬件可用性 / 兴趣选

### 一句话总结每个里程碑的功能

| 里程碑 | 一句话功能 |
|--------|-----------|
| M1 ✅ | 能跑通 Qwen3 单条推理，结果和 transformers 一致 |
| M2 ✅ | 单条推理快 7×（KV Cache 让 decode 不重算历史） |
| M3 🟡 | 8 个请求同时生成，短的不等长的，slot 自动复用 |
| M4 ⬜ | 显存不浪费，长 prompt + 多并发不 OOM，前缀能复用 |
| M5·P1 ⬜ | `inferlite serve` 起服务，curl 流式输出像 ChatGPT |
| M5·P2 ⬜ | 多轮对话第二轮秒回（前缀缓存命中） |
| M5·P3 ⬜ | 有对照表证明我们快，CI 绿勾，打 v1.0 tag |
| M6 ⬜ | 能跑 MoE 模型（Qwen3-30B-A3B，激活 3B 参数） |
| M7 ⬜ | 长续写快 1.5×（n-gram spec decoding，不训练） |
| M8 ⬜ | 性能逼近 vLLM（Triton kernel，需 GPU） |
| M9 ⬜ | MoE 快 5+ tok/s（grouped GEMM） |
| M10 ⬜ | Spec 快 3×（训 draft head，EAGLE） |
| M11 ⬜ | 32K+ prompt 不 OOM（分块预填） |
| M12 ⬜ | 上下文 32K → 128K（YaRN RoPE 缩放） |
| M13 ⬜ | 能看图（图文对话） |
| M14 ⬜ | 图片多分辨率 + 前缀缓存 + 异步 |
