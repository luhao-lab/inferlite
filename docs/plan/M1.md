...
QwenLM/Qwen3、llama.cpp、SGLang、TGI、TensorRT-LLM 统一记录在 `docs/knowledge/knowledge.md`；M1 只按上表读取，不泛读。
...
参考卡片：[`knowledge.md` → `vLLM — Qwen3 weight loading`](../knowledge/knowledge.md#vllm-qwen3-weight-loading)。
...
> 概念（形状/超参/upcast/tie_embed 等）见 `docs/knowledge/knowledge.md` → Concepts。
> 库 API（transformers/pytest/uv 等）见 `docs/knowledge/knowledge.md` → Libraries。
> 论文（RMSNorm/SwiGLU/RoPE/GQA）见 `docs/knowledge/knowledge.md` → Papers。

## 12. 参考项目与论文

> 见 [docs/knowledge/knowledge.md](../knowledge/knowledge.md) — 三梯队开源项目（nano-vllm / LLMs-from-scratch / transformers / vllm / ...）+ 论文清单 + 阶段-资料对照表。
